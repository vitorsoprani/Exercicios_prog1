...
float calculaVantagem(Pokemon atacante, Pokemon defensor) { //deve dar pra otimizar, mas sem tempo irmao
...
Pokemon subirDeNivel(Pokemon p) {
    ...
    p.vidaMaxima += floor(p.vidaMaxima * 0.1);
...
Ginasio RealizaBatalha(Ginasio ginasio) {
    while (estaVivo(ginasio.pokemonDiogo) && ginasio.batalhaAtual < ginasio.qtdDeAdversarios) {
        ginasio = batalhar(ginasio.pokemonDiogo, ginasio.adversarios[ginasio.batalhaAtual], ginasio);
    }
    return ginasio;
}
...
void FinalizaBatalha(Ginasio ginasio) {
    if (estaVivo(ginasio.pokemonDiogo)) {
        ...
        imprimePokemon(ginasio.adversarios[ginasio.batalhaAtual - 1]);
    }
}